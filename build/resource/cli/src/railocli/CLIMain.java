 ...
 */
	...
	public static void main(String[] args) throws ServletException, IOException, JspException {
		...
		if(Util.isEmpty(strRoot,true)) throw new IOException("missing webroot");
		File root=new File(strRoot);
		root.mkdirs();

		String strServerroot=config.get("server-config");
		if(Util.isEmpty(strServerroot,true)) throw new IOException("missing server configuration");
		File serverRoot=new File(strServerroot);
		serverRoot.mkdirs();
		
		String strWebroot=config.get("web-config");
		if(Util.isEmpty(strWebroot,true)) throw new IOException("missing web configuration");
		File webRoot=new File(strWebroot);
		webRoot.mkdirs();
